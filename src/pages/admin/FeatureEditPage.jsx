...
export default function FeatureEditPage() {
  ...
  useEffect(() => {
    API.get(`/api/features/editForm/${id}`, {
  ...
  const handleSubmit = () => {
    ...
    API.patch(`/api/features/${id}`, updates, {
      headers: {
        Authorization: `Bearer ${token}`,
      }
    })
      .then(() => navigate(`/admin/features/view/${id}`))
  ...
  const otherFields = Object.keys(record)
    .filter(
      (k) =>
        !["featureId", "featureName", "moduleName"].includes(k) &&
  ...
  return (
    <div className="container">
      ...
      <h3 className="text-center">Edit Feature</h3>
